...
def get_remote_context(check_name, file_name):
    ...
    try:
        resp = requests.get(
            url="https://github.com/hellowjjb/mydocker/raw/master/qqjscript/"+file_name, timeout=10)
    except RequestException as error:
        logger.warning(f"获取{check_name}内容时发生网络请求异常，尝试使用镜像仓库")
        try:
            resp = requests.get(
                url="https://raw.fastgit.org/hellowjjb/mydocker/master/qqjscript/"+file_name, timeout=10)
